...
* [`src/re_frame01/`](src/re_frame01/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/re_frame01/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `re-frame01` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame01/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame01.example
  (:require [re-frame01.config :as config])
